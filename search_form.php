<?php
	...
	$result1 = $mysqli->query("SELECT * FROM genres;");
	if( !$result1) {
		echo $mysqli->error;
		exit();
	}

	$result2 = $mysqli->query("SELECT * FROM ratings;");
	if( !$result2) {
		echo $mysqli->error;
		exit();
	}

	$result3 = $mysqli->query("SELECT * FROM labels;");
	if( !$result3) {
		echo $mysqli->error;
		exit();
	}

	$result4 = $mysqli->query("SELECT * FROM formats;");
	if( !$result4) {
		echo $mysqli->error;
		exit();
	}

	$result5 = $mysqli->query("SELECT * FROM sounds;");
	if( !$result5) {
		echo $mysqli->error;
		exit();
	}
